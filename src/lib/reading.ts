export function calculateReadingTimeFromHtml(html: string, wordsPerMinute = 200, minTime = 1): number {
  if (!html) return minTime;
  const text = html
  ...
  return Math.max(minTime, Math.ceil(words / wordsPerMinute));
}
